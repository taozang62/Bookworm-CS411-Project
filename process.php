...
if ( isset($_POST['search'])) {
  ...
  $result = $conn->query("SELECT count(*) as cnt FROM projectDatabase.books where name like '%$input%'") or  die(mysqli_error($conn));
...
if ( isset( $_POST['save']) ){
    ...
    $au_name = $_REQUEST['author'];
    $rating = $_REQUEST['rating'];
    if (!empty($book_name) && !empty($au_name) && !empty($rating)){
      ...
      $sql = "SELECT EXISTS(SELECT author, name FROM projectDatabase.books WHERE author = '$au_name' AND name = '$book_name') AS num";
      $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
      $row = mysqli_fetch_array($res, MYSQLI_ASSOC);
      $b_flag = $row['num'];

      $sql = "SELECT MAX(book_id) as maximum FROM projectDatabase.books";
      $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
      $row = mysqli_fetch_array($res, MYSQLI_ASSOC);
      $new_b_id = -1;
      $new_b_rating = -1;
      ...
      if ($b_flag == 0){
        ...
        $sql = "SELECT book_id, rating, rating_cnt FROM projectDatabase.books WHERE name = '$book_name'";
        $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
        $row = mysqli_fetch_array($res, MYSQLI_ASSOC);
        $new_b_id = $row['book_id'];
      ...
      if ($flag == 0){
        ...
        $sql = "UPDATE projectDatabase.author SET name = '$au_name' WHERE author_id = '$new_id'";
        $res = mysqli_query($conn, $sql) or die(mysqli_error($conn));
      }
  ...
  if ( isset( $_GET['delete'])){
    $id = $_GET['delete'];
    $result = $conn->query("SELECT author as au FROM projectDatabase.books WHERE book_id = $id") or die($conn->error);
    $row = $result->fetch_array();
    $author_name = $row['au'];
    $result = $conn -> query("SELECT author_id as aid FROM projectDatabase.author WHERE name = '$author_name'") or die($conn->error);
    $row = $result->fetch_array();
    $aid = $row['aid'];
    $conn->query("DELETE FROM projectDatabase.books WHERE book_id = $id") or die($conn->error);
    $result = $conn->query("SELECT count(*) as cnt FROM projectDatabase.books WHERE author = '$author_name'");
    $row = $result->fetch_array();
    $cnt = $row['cnt'];
    if ($cnt == 0){
...
//update not working
  if ( isset( $_GET['edit'])){
    $id = $_GET['edit'];
    $update = true;
    $result = $conn->query("SELECT * FROM projectDatabase.books WHERE book_id = $id") or die($conn->error);
    // print($id);
    if(!empty($result)){
        $row = $result->fetch_array();
        $book_id = $row['book_id'];
  ...
  if ( isset( $_POST['update'])){
    ...
    $author = $_POST['author'];
    $rating = $_POST['rating'];
    $conn->query("UPDATE projectDatabase.books SET name = '$name', author = '$author', rating = '$rating' WHERE book_id = $book_id") or die($conn->error);
    $conn->query("UPDATE projectDatabase.author SET name = '$author' WHERE name = '$old_name'") or die($conn->error);
